...
void limits_init()
{
	...
	GPIOPadConfigSet(LIMIT_BASE, LIMIT_MASK, GPIO_STRENGTH_2MA, GPIO_PIN_TYPE_STD_WPD);
#else
	// Weak Pull-Up on pins, pins are 1 state until connected to Vss
	GPIOPadConfigSet(LIMIT_BASE, LIMIT_MASK, GPIO_STRENGTH_2MA, GPIO_PIN_TYPE_STD_WPU);
#endif

	if (bit_istrue(settings.flags,BITFLAG_HARD_LIMIT_ENABLE))
	{
		GPIOIntTypeSet(LIMIT_BASE, LIMIT_MASK, GPIO_BOTH_EDGES);
		GPIOPinIntEnable(LIMIT_BASE, LIMIT_MASK);
		IntEnable(LIMIT_INT);
	}
	else
	{
...
void limits_disable()
{
	GPIOPinIntDisable(LIMIT_BASE, LIMIT_MASK); // Disable specific pins of the Pin Change Interrupt
	IntDisable(LIMIT_INT); // Disable Pin Change Interrupt
}
