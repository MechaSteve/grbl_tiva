...
int limits_get_state()
{
  int limit_state = 0;
  // For convienience, the lower byte is the non-directional limit state ( pos OR neg )
  // second byte is positive limits
  // third byte is negative limits
  if(x_positive_limit_get()) limit_state |= X_AXIS_MASK + (X_AXIS_MASK <<  8);
  if(x_negative_limit_get()) limit_state |= X_AXIS_MASK + (X_AXIS_MASK << 16);
  if(y_positive_limit_get()) limit_state |= Y_AXIS_MASK + (Y_AXIS_MASK <<  8);
  if(y_negative_limit_get()) limit_state |= Y_AXIS_MASK + (Y_AXIS_MASK << 16);
  if(z_positive_limit_get()) limit_state |= Z_AXIS_MASK + (Z_AXIS_MASK <<  8);
  if(z_negative_limit_get()) limit_state |= Z_AXIS_MASK + (Z_AXIS_MASK << 16);
