...
typedef struct {
  int state;               // Tracks the current system state of Grbl.
  int abort;               // System abort flag. Forces exit back to main loop for reset.
  int suspend;             // System suspend bitflag variable that manages holds, cancels, and safety door.
  int soft_limit;          // Tracks soft limit errors for the state machine. (boolean)
  int step_control;        // Governs the step segment generator depending on system state.
  int probe_succeeded;     // Tracks if last probing cycle was successful.
  int homing_axis_lock;    // Locks axes when limits engage. Used as an axis motion mask in the stepper ISR.
  int f_override;          // Feed rate override value in percent
  int r_override;          // Rapids override value in percent
  int spindle_speed_ovr;   // Spindle speed value in percent
  int spindle_stop_ovr;    // Tracks spindle stop override states
  int report_ovr_counter;  // Tracks when to add override data to status reports.
  int report_wco_counter;  // Tracks when to add work coordinate offset data to status reports.
...
volatile int sys_position[N_AXIS];      // Real-time machine (aka home) position vector in steps.
int sys_probe_position[N_AXIS]; // Last probe position in machine coordinates and steps.

int sys_probe_state;   // Probing state value.  Used to coordinate the probing cycle with stepper ISR.
int sys_rt_exec_state;   // Global realtime executor bitflag variable for state management. See EXEC bitmasks.
int sys_rt_exec_alarm;   // Global realtime executor bitflag variable for setting various alarms.
int sys_rt_exec_motion_override; // Global realtime executor bitflag variable for motion-based overrides.
int sys_rt_exec_accessory_override; // Global realtime executor bitflag variable for spindle/coolant overrides.
...
float system_convert_axis_steps_to_mpos(volatile int *steps, uint8_t idx);

// Updates a machine 'position' array based on the 'step' array sent.
void system_convert_array_steps_to_mpos(float *position, volatile int *steps);
