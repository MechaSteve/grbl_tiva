  ...
  uint8_t axis_0, uint8_t axis_1, uint8_t axis_linear, uint8_t is_clockwise_arc)
{
  ...
  //plan_line_data_t local_pl_data = pl_data;

  //TODO Remove Debug Statement
  //printPgmString(("mc_arc"));
...
void mc_dwell(float seconds)
{
  ...
  //TODO: Replace this with a dedicated mc instruction
  delay_sec(seconds, DELAY_MODE_DWELL);
}


//TODO: Replace this one functions with three.
// mc_homing_seek( float target[N_AXIS], float feedrate)  target is maximum distance to travel. non-zero selects axis, +/- is direction. event is not recorded
// mc_homing_on( float target[N_AXIS], float feedrate) target is maximum distance to travel. non-zero selects axis, +/- is direction
// mc_homing_off( float target[N_AXIS], float feedrate) target is distance to travel off. non-zero selects axis, +/- is direction
// global arrays: int limit_on_event[N_HOMING_CYCLES][N_AXIS], limit_off_event  : to be used for reporting and repeat ability requirements
