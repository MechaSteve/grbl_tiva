...
#define AMASS_LEVEL1 (SysCtlClockGet()/8000) // Over-drives ISR (x2). Defined as F_CPU/(Cutoff frequency in Hz)
#define AMASS_LEVEL2 (SysCtlClockGet()/4000) // Over-drives ISR (x4)
#define AMASS_LEVEL3 (SysCtlClockGet()/2000) // Over-drives ISR (x8)
...
typedef struct {
  ...
  int direction_bits;
    ...
    int is_RPM_rate_adjusted; // Tracks motions that require constant laser power/rate
  #endif
} st_block_t;
...
typedef struct {
  int n_step;           // Number of step events to be executed for this segment
  int cycles_per_tick;  // Step distance traveled per ISR tick, aka step rate.
  int  st_block_index;   // Stepper block data index. Uses this information to execute this segment.
  #ifdef ADAPTIVE_MULTI_AXIS_STEP_SMOOTHING
    int amass_level;    // Indicates AMASS level for the ISR to execute this segment
  #else
...
typedef struct {
  ...
  int execute_step;     // Flags step execution for each interrupt.
  int step_pulse_time;  // Step pulse reset time after step rise
  int step_outbits;         // The next stepping-bits to be output
  int dir_outbits;
  #ifdef ADAPTIVE_MULTI_AXIS_STEP_SMOOTHING
    uint32_t steps[N_AXIS];
  #endif

  int step_count;       // Steps remaining in line segment motion
  int exec_block_index; // Tracks the current st_block index. Change indicates new block.
...
static volatile int segment_buffer_tail;
static int segment_buffer_head;
static int segment_next_head;

// Step and direction port invert masks.
static int step_port_invert_mask;
static int dir_port_invert_mask;
...
void st_go_idle()
{
	...
	if (pin_state) { AxisMasterDisable();  }
	else { AxisMasterEnable();  }
}
...
////
			...
			st_go_idle();
#ifdef VARIABLE_SPINDLE
			// Ensure RPM is set properly upon completion of rate-controlled motion.
			if (st.exec_block->is_RPM_rate_adjusted) { spindle_set_speed(0); }
#endif
	...
	if (st.counter_x > st.exec_block->step_event_count) {
		st.step_outbits |= (X_AXIS_MASK);
		st.counter_x -= st.exec_block->step_event_count;
		if (st.exec_block->direction_bits & (X_AXIS_MASK)) { sys_position[X_AXIS]--; }
	...
	if (st.counter_y > st.exec_block->step_event_count) {
		st.step_outbits |= (Y_AXIS_MASK);
		st.counter_y -= st.exec_block->step_event_count;
		if (st.exec_block->direction_bits & (Y_AXIS_MASK)) { sys_position[Y_AXIS]--; }
	...
	if (st.counter_z > st.exec_block->step_event_count) {
		st.step_outbits |= (Z_AXIS_MASK);
		st.counter_z -= st.exec_block->step_event_count;
		if (st.exec_block->direction_bits & (Z_AXIS_MASK)) { sys_position[Z_AXIS]--; }
...
//}
void OnStepReset(void)
{
	AxisStepSet(step_port_invert_mask);
}
#ifdef STEP_PULSE_DELAY
  ...
  //}
	void OnStepDelay(void)
	{
		AxisStepSet(st.step_bits);
	}
#endif
...
void st_generate_step_dir_invert_masks()
{
  ...
  //}
    ...
    //Simplification due to using internal masks only
...
void st_reset()
{
  ...
  AxisStepSet(step_port_invert_mask);
  AxisDirectionSet(dir_port_invert_mask);
}
...
void st_prep_buffer()
{
  ...
  while (segment_buffer_tail != segment_next_head) { // Check if we need to fill the buffer.
    ...
    */
    ...
    #ifdef VARIABLE_SPINDLE
      /* -----------------------------------------------------------------------------------
        Compute spindle speed RPM output for step segment
      */
      
      if (st_prep_block->is_RPM_rate_adjusted || (sys.step_control & STEP_CONTROL_UPDATE_SPINDLE_RPM)) {
        if (pl_block->condition & (PL_COND_FLAG_SPINDLE_CW | PL_COND_FLAG_SPINDLE_CCW)) {
          float rpm = pl_block->spindle_speed;
          // NOTE: Feed and rapid overrides are independent of PWM value and do not alter laser power/rate.        
          if (st_prep_block->is_RPM_rate_adjusted) { rpm *= (prep.current_speed * prep.inv_rate); }
          // If current_speed is zero, then may need to be rpm_min*(100/MAX_SPINDLE_SPEED_OVERRIDE)
          // but this would be instantaneous only and during a motion. May not matter at all.
          prep.current_spindle_RPM = spindle_compute_hz_value(rpm);
        } else { 
          sys.spindle_speed = 0.0;
          prep.current_spindle_RPM = 0;
        }
        bit_false(sys.step_control,STEP_CONTROL_UPDATE_SPINDLE_RPM);
      }
      prep_segment->spindle_RPM = prep.current_spindle_RPM; // Reload segment RPM value
    ...
    */
    ...
    uint32_t cycles = ceilf( (80.0*1000000.0*60.0)*inv_rate ); // (cycles/step)
