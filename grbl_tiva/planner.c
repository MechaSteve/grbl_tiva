...
static int block_buffer_tail;     // Index of the block to process now
static int block_buffer_head;     // Index of the next block to be pushed
static int next_buffer_head;      // Index of the next buffer head
static int block_buffer_planned;  // Index of the optimally planned block
...
uint8_t plan_check_full_buffer()
{
  ...
  if (block_buffer_tail == plan_next_block_index(next_buffer_head) ) { return(true); }
...
uint8_t plan_buffer_line(float *target, plan_line_data_t *pl_data)
{
  ...
  for (idx=0; idx<N_AXIS; idx++) {
    ...
    if (delta_mm < 0.0 ) { block->direction_bits |= get_axis_mask(idx); }
  }
